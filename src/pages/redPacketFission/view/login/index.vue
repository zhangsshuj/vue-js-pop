<template>
    <div class="login-box">
        ...
        <div class="content">
            <input type="text" v-wxBlur class="phone" placeholder="请输入手机号码" oninput='if(value.length > 11) value = value.slice(0, 11)' v-model="number" :disabled="numberFlag" />
        ...
        async handleGetCode() {
            try {
                if (!this.number) {
                    ...
                    let data = await this.$api.getCode(params);
                    if (data.head.CODE == "0") {
                        this.Loading.close();
