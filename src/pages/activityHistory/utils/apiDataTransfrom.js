...
export default function (list) {
    ...
    let transformGrupMonth = (list, index) => {
        let month = "";
        let currTime = getYearMonth(list[index].flowCreateTime);
        if (index === 0) {
            month = currTime;
        } else {
            let perTime = getYearMonth(list[index - 1].flowCreateTime);
    ...
    return list.map((item, index) => {
        ...
        // 订单金额
        newItem["teadeAmount"] = this.$utils.moneyFormatCN(item.teadeAmount, true); // true代表元 false代表分
        // 活动投资时间 到日
        // newItem["orderTime"] = item.flowCreateDate;
        // 活动投资时间 到分
        newItem["flowCreateTime"] = item.flowCreateTime;
