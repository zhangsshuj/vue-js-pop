...
export default {
  ...
  methods: {
    ...
    */
     ...
     */
    tabClick (tab, index) {
      if (tab.orderStatus === this.query.orderStatus || this.loading.isShow) return;
    ...
    orderItemClick (item) {
      ...
      this.$router.push({ path: "/detail", query: { orderData: encodeOrder } });
      // 使用投资时间作为唯一ID 已读标记
